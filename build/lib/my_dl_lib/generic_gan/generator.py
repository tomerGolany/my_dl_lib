...
def generate_data_from_trained_generator(num_of_samples_to_generate, meta_file_name,
    ...
    generator_input = np.random.normal(-1, 1, [num_of_samples_to_generate, 100])
    # Restore the graph:
    with tf.Session() as sess:
        ...
        if not is_conditional_gan:
            input_placeholder = graph.get_tensor_by_name('generator/input_placeholder:0')
        ...
        data = data.reshape(num_of_samples_to_generate, 240)
